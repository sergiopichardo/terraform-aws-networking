locals {
  public_subnets = {
    // returns only the public subnets
...
// returns whatever availability zones are available in the region
...
resource "aws_internet_gateway" "this" {
  // if the number of public_subnets > 0, then create 1 Internet Gateway. Otherwise, don't create one. 
